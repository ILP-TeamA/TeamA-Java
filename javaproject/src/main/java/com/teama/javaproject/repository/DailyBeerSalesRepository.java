...
public interface DailyBeerSalesRepository extends JpaRepository<DailyBeerSales, Long> {
       ...
       /**
        * 指定したsales_idの売上データを取得
        */
       List<DailyBeerSales> findBySalesId(Integer salesId);

       /**
        * 指定したsales_idの売上データを商品ID順で取得
        */
       List<DailyBeerSales> findBySalesIdOrderByProductId(Integer salesId);
